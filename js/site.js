const buildADeck = {
  ...
  appendTableRows() {
    ...
    for(i = 0; i < this.json.length; i++) {
      var table_row = this.json[i];
      var row = table_body.insertRow();
      var cell0 = row.insertCell();
      cell0.innerHTML = '<td>'+(i+1)+'</td>' ;
      var cell1 = row.insertCell();
      cell1.innerHTML = '<td>'+this.json[i].Category+'</td>' ;
      var cell2 = row.insertCell();
      cell2.innerHTML = '<td>' + this.json[i].Name+'</td>';
      var cell3 = row.insertCell();
      if (this.json[i].url.length > 0){
          cell3.innerHTML = '<td><a href="'+this.json[i].url+'">'+this.json[i].Desc+'</a></td>';
      } else {
          cell3.innerHTML = '<td>'+this.json[i].Desc+'</td>';
      }
